  ...
  try {
    ...
    if (!studentId || !currentEmail) {
      return NextResponse.json(
        { error: "studentId y currentEmail son requeridos" },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      // Verificar que el UID coincida con el studentId
      if (userRecord.uid !== studentId) {
        return NextResponse.json(
          { error: "El ID del estudiante no coincide con el email proporcionado" },
          { status: 400 }
        );
      }
