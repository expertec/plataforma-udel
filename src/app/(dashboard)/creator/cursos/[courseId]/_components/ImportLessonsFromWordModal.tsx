...
const mergeUniqueWarnings = (current: string[], next: string[]): string[] =>
...
const previewLessonKey = (lesson: ParsedLessonFromFile): string =>
  `${lesson.sourceFileKey}:${lesson.sourceLessonIndex}:${lesson.title}`;
...
export function ImportLessonsFromWordModal({
  ...
  const parseFiles = async (files: File[]) => {
    ...
    for (const file of docxFiles) {
      ...
      if (existingKeys.has(key) || seenInBatch.has(key)) {
        duplicateWarnings.push(`${file.name}: esta semana ya estaba agregada y se omitió.`);
        continue;
      }
      seenInBatch.add(key);
    ...
    try {
      ...
      const parseWarnings: string[] = [...duplicateWarnings];
      ...
      if (nextLessons.length === 0) {
        setWarnings((current) => mergeUniqueWarnings(current, parseWarnings));
        setParseError("No fue posible extraer semanas válidas de los archivos Word seleccionados.");
        return;
      }

      setSelectedFiles((current) => [...current, ...filesToParse]);
      setParsedLessons((current) => [...current, ...nextLessons]);
      setExpandedPreviewLessons((current) => {
        const next = new Set(current);
        nextLessons.forEach((lesson) => {
          next.add(previewLessonKey(lesson));
        });
        return next;
      });
      setWarnings((current) => mergeUniqueWarnings(current, parseWarnings));
      toast.success(
        `Se agregaron ${nextLessons.length} lección(es) y ${totalClassesCount(nextLessons)} clase(s).`,
      );
    } catch (error) {
      console.error(error);
  ...
  const handleRemoveFile = (targetFile: File) => {
    ...
    setSelectedFiles((current) => current.filter((file) => fileKey(file) !== targetKey));
    setParsedLessons((current) => current.filter((lesson) => lesson.sourceFileKey !== targetKey));
    setExpandedPreviewLessons((current) => {
      const next = new Set(current);
      ...
      <div className="w-full max-w-5xl max-h-[calc(100vh-3rem)] overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(300px,340px)]">
          <div className="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4">
            ...
            <div className="mt-2 flex flex-col items-center justify-center gap-2 rounded-lg border-2 border-dashed border-slate-300 bg-white px-4 py-6 text-center text-sm text-slate-500 transition hover:border-blue-400">
              <input
                ref={inputRef}
                type="file"
              ...
              <div className="w-full">
                <p className="font-semibold text-slate-800">
                  {selectedFiles.length > 0
                    ? `${selectedFiles.length} semana(s) cargada(s)`
            ...
            {selectedFiles.length > 0 ? (
              <div className="rounded-lg border border-slate-200 bg-white p-3">
                <div className="mb-3 flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-900">Plantillas agregadas</p>
                  <p className="text-xs text-slate-500">
                    Quitar una semana elimina sus lecciones detectadas de la vista previa.
                  </p>
                </div>
              ...
              <div className="rounded-lg border border-slate-200 bg-white">
                ...
                <div className="max-h-[28rem] space-y-3 overflow-auto p-3">
                  {parsedLessons.map((lesson, lessonIndex) => {
                    const lessonKey = previewLessonKey(lesson);
